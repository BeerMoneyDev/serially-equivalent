/**
 * Frozen copy of main @6622ee8 for differential testing — never edit.
 *
 * This is `src/serially-equivalent.ts` exactly as it stood on upstream `main`
 * (merge of PR #2, before the comparison fast-path rewrite), with only the
 * export renamed to `referenceSeriallyEquivalent`. The differential and
 * FHIR-shape specs run every input through both this reference and the live
 * implementation and require identical results (and an identical first debug
 * message, modulo the rewrite visiting keys in insertion rather than sorted
 * order). It is excluded from the published build (tsconfig.build.json).
 */
/* eslint-disable */
 ...
 */
export function referenceSeriallyEquivalent<T = any>(
  a: T,
  b: T,
  options?: SeriallyEquivalentOptions,
): boolean {
